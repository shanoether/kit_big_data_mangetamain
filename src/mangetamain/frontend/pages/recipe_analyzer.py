...
class RecipeAnalyzer:
    ...
    def plot_contributions_by_semester(self, start_year: int, end_year: int):
        ...
        if cache_key not in self._cache:
            df = (
                self.df_interaction.with_columns(
                    [
    ...
    def plot_top_ingredients(self, top_n: int = 20):
        ...
        if cache_key not in self._cache:
            ...
            ingredients_cleaned = (
                ...
                .select(
                    pl.col("cleaned").str.split(", ").explode().alias("ingredient"),
                )
                .filter(
            ...
            counts = (
                ingredients_cleaned.group_by("ingredient")
